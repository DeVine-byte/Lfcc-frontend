    ...
    return (
      <div className="bg-black min-h-screen flex items-center justify-center text-white">
        <div className="text-center">
          ...
          <p className="text-zinc-400">Loading broadcast...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-black min-h-screen flex items-center justify-center px-4">
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8 text-center max-w-md">
          <h2 className="text-2xl font-bold text-red-400 mb-4">Broadcast Not Found</h2>
          <p className="text-zinc-400 mb-6">{error}</p>
          <button onClick={() => navigate("/")} className="bg-purple-500 hover:bg-purple-600 transition px-6 py-3 rounded-xl font-semibold text-white">
            Go Home
          </button>
        </div>
      </div>
      ...
      <nav className="border-b border-zinc-800 bg-black/90 backdrop-blur-md sticky top-0 z-50">
        ...
        {mobileMenu && (
          <div className="md:hidden border-t border-zinc-800 px-4 py-4 flex flex-col gap-4 text-zinc-300">
            ...
            <Link to="/about" onClick={() => setMobileMenu(false)} className="hover:text-purple-400 transition">About</Link>
          </div>
        )}
      </nav>
